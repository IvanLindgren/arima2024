...
def arima_forecast_and_plot(data_source, column_name, forecast_period=5, train_period_months=2):
    """
    ...
    if data_source == 'kran15_rez':
        df = read_kran15_rez(file_path=f'Кран/Rez/rez_month.xlsx')  # Заменить на корректный путь к файлу
        if df is not None:
    ...
    results = {
        ...
        "plot_path": "forecast_plot.png",  # Путь к сохраненному графику
...
results = arima_forecast_and_plot(data_source='kran_15_rez', column_name='Аварийный останов', forecast_period=5)
