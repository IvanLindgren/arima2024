...
def get_kran_15_rez_data(file_path: str) -> dict:
    ...
    plots = dict()
    plots['Общий график'] = create_general_graf(allGr)
    plots['Сезонные графики'] = create_seasonal_graf(allGr)
    plots['Построение скользящего среднего'] = create_moving_average_graf(allGr)
    plots['График автокорелляции'] = create_autocor_graf(allGr)
    ...
    for name, time_series in allGr.items():
        ...
        name_data['stationarity'] = f"Анализ для {name}: {check_stationarity(time_series['Counts'])}"
        
        # Подбор параметров модели и расчет метрик
        best_order, metrics = tune_arima_with_grid_search(time_series['Counts'])
        name_data['parametrs'] = f"Лучшие параметры: ARIMA{best_order} с метриками: {metrics}"
       
        # Обучение модели и прогнозирование
        forecast, metrics, fig = train_and_forecast_with_metrics(time_series['Counts'], order=best_order)
        name_data['plot'] = fig
